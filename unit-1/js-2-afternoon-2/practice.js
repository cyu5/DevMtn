...
//Code Here
const divider = arr => {
  return [arr.filter(num => num % 2 ===0), arr.filter(num => num % 2)];
}
...
const addTen = numbers => {
  return numbers.map(el => Number(el) + 10);
}
...
const both = (arr1, arr2) => {
  ...
  let intersection = bothArr.filter(function(elem, index, self) {
...
//Code Here
devMountainEmployees = devMountainEmployees.filter(person => person.name != "Cahlan");
